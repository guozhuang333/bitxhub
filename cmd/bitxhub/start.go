...
func start(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("调用上传一半456HandleTransaction出错%v", err)
	}
	...
	err = api.Broker().HandleTransaction(collect456ShareTx)
	if err != nil {
		return fmt.Errorf("调用上传完整456HandleTransaction出错%v", err)
	}
